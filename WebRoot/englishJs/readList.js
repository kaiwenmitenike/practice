...
function queryEnglishWord(pageNum,pageCount){
	$.ajax({
		url:"article/english",
		data:{"pageNum":pageNum,"pageCount":pageCount,"title":$("#title").val(),"author":$("#author").val()},
		type:"GET",
		dataType:"json",
		success:function(data){
			...
			$.each(data.list,function(e,info){
				_html +='<tr>'+
						'<th>'+info.title+'</th>'+
						'<th><div class="divcss">'+info.article+'</div></th>'+
						'<th>'+info.author+'</th>'+
						'<th>'+info.add_time+'</th>'+
						'<th><button class="je-btn je-bg-green je-btn-small" value="查看详情" onclick="queryArticleById(\''+info.id+'\');"><i class="je-icon"></i></button>'+
						'<button class="je-btn je-bg-green je-btn-small" value="修改文章" onclick="UpdateReadArticleById(\''+info.id+'\');"><i class="je-icon"></i></button>'+
...
function deleteExerises(exercise_id){
	...
	if(yesNo==true){
		$.ajax({
			url:"article/english/"+exercise_id,
			type:"DELETE",
			dataType:"json",
			success:function(data){
				if(data.status==200){
...
function queryArticleById(protypeId){
	this.location.href=$("#context_path").val()+"/readArticle.jsp?id="+protypeId;
}
function UpdateReadArticleById(protypeId){
	this.location.href=$("#context_path").val()+"/addArticle.jsp?id="+protypeId;
}
